...
__author__ = 'Alexander and Solveig'
...
def main():
    ...
    #creates folder for all .jpg and .jpeg files:
    pictuer_folder = os.path.join(path, 'pictures')
    if not os.path.exists(pictuer_folder):
        os.mkdir(pictuer_folder)
        print('pic was created')
    else:
        print('pic exists!')
    ...
    for root, dirs, files in os.walk(path):
        for name in files:
            ...
            if splittedName.endswith("nfo") or splittedName.endswith("sfv") :
                ...
                shutil.move(src, des)
            elif splittedName.endswith('jpeg') or splittedName.endswith('jpg'):
                src = os.path.join(root, name)
                des = os.path.join(pictuer_folder, name)
                shutil.move(src, des)
